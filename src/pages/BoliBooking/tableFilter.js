...
import { appAllGetBoliBooking, appDeleteBoliBooking, appUpdateBoliBooking } from "../../store/boliBooking";
...
function TableFilter() {
  ...
  const { bolibookinglist } = useSelector((state) => state.bolibookingReducer);
  const paginate = useSelector((state) => state.bolibookingReducer.paginate);
  const isloder = useSelector((state) => state?.bolibookingReducer.isloder);
  ...
  useEffect(() => {
    ...
    dispatch(appAllGetBoliBooking({ page: 1, limit: 10 }));
  }, []);
  ...
  useEffect(() => {
    ...
    dispatch(appAllGetBoliBooking(searchParams));
    navigate(`/boli-booking/${page}`);
  }, [page, activeFilters]);
  ...
  const handleDeleteSocialProjectBooking = (id) => {
    swal({
      title: "Are you sure?",
      text: "You want to delete this Boli Booking?",
      icon: "warning",
      buttons: true,
      dangerMode: true,
    }).then((willDelete) => {
      if (willDelete) {
        dispatch(appDeleteBoliBooking(id)).then(() => {
          dispatch(appAllGetBoliBooking({ page: 1, limit: 10 }));
        });
      }
    });
  ...
  const handleAcceptedBoliBooking = (id) => {
    swal({
      title: "Are you sure?",
      ...
      if (willAccept) {
        dispatch(
          appUpdateBoliBooking({ id, status: "Accepted" })
        ).then(() => {
          dispatch(appAllGetBoliBooking({ page: 1, limit: 10 }));
        });
      }
  ...
  const handleRejectedBoliBooking = (id) => {
    setRejectedId(id);
        ...
        <div className="card-body">
          ...
          <div className="table-responsive bootgrid">
            <table className="table table-hover" id="bootgrid-command">
              ...
              {bolibookinglist?.data &&
                bolibookinglist?.data.map((value, index) => {
                  return (
                    <tbody key={index}>
                      <tr>
                        <td>
                        ...
                        <td>{value?.event?.category}</td>
                        <td>{value?.boli_item}</td>
                        <td>
                          <img
                          ...
                          {value.status === "Pending" && (
                            <div style={{ display: "flex", gap: "5px" }}>
                              <button
                                ...
                                onClick={() =>
                                  handleAcceptedBoliBooking(value?._id)
                                }
                              >
                              ...
                              <button
                                ...
                                onClick={() =>
                                  handleRejectedBoliBooking(value?._id)
                                }
                              >
