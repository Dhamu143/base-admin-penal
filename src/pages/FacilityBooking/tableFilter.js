...
import {
  appAllGetFacilityBooking,
  appDeleteFacilityBooking,
  appUpdateFacilityBooking,
} from "../../store/facilitybooking";
...
function TableFilter() {
  ...
  const paginate = useSelector(
    (state) => state.facilitybookingReducer.paginate
  );
  ...
  useEffect(() => {
    ...
    dispatch(appAllGetFacilityBooking(searchParams));
    navigate(`/facility-booking/${page}`);
  }, [page, activeFilters]);
  ...
  const handleDeleteFacilityBooking = (id) => {
    swal({
      title: "Are you sure?",
      text: "You want to delete this Facility Booking?",
      icon: "warning",
      buttons: true,
      dangerMode: true,
    }).then((willDelete) => {
      if (willDelete) {
         dispatch(appDeleteFacilityBooking(id))
         .then(() => {
  ...
  const handleAcceptedFacilityBooking = (id) => {
    swal({
      title: "Are you sure?",
      ...
      if (willAccept) {
        dispatch(appUpdateFacilityBooking({ id, status: "Accepted" })).then(
          () => {
            dispatch(appAllGetFacilityBooking({ page: 1, limit: 10 }));
          }
        );
  ...
  const handleRejectedFacilityBooking = (id) => {
    setRejectedId(id);
    ...
    {error && <span className="text-danger">{error}</span>}
          ...
          <div className="table-responsive bootgrid">
            <table className="table table-hover" id="bootgrid-command">
              ...
              {facilitylist?.data &&
                facilitylist?.data.map((value, index) => {
                  return (
                    <tbody key={index}>
                      <tr>
                        <td>
                        ...
                        <td>
                          ...
                          <br />
                          {value?.paymentMode && (<small>Mode: {value?.paymentMode}</small>)}
                        </td>
                        <td>
                        ...
                        <td>
                          {value.status === "Pending" && (
                            <div style={{ display: "flex", gap: "5px" }}>
                              <button
                                ...
                                onClick={() =>
                                  handleAcceptedFacilityBooking(value?._id)
                                }
                              >
                              ...
                              <button
                                ...
                                onClick={() =>
                                  handleRejectedFacilityBooking(value?._id)
                                }
                              >
                          ...
                          {value.status === "Rejected" && (
                            <button
                              ...
                              className="btn btn-sm btn-danger command-delete"
                              onClick={() =>
                                handleDeleteFacilityBooking(value?._id)
                              }
                            >
