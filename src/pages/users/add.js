...
const languages = useSelector((state) => state?.languageReducer?.language) || [];
...
const caste = useSelector((state) => state?.casteReducer?.caste) || []; 
  ...
  useEffect(() => {
    if (caste && id) {
      const selectedCaste = Array.isArray(caste) ? caste.find(c => c._id === id) : caste;
      if (selectedCaste) {
        formik.setFieldValue("name", selectedCaste?.name);
        formik.setFieldValue("languages_id", selectedCaste?.language?.map(lang => ({
          value: lang._id,
          label: lang.name
        })) || []);
        formik.setFieldValue("nativeplace_id", selectedCaste?.nativeplace?.map(place => ({
          value: place._id,
          label: place.name
        })) || []);
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      nativeplace_id: Yup.array().min(1, "At least one native place is required"),
      ...
      <div className="content-wrapper">
        <div className="content-heading">
          <div>
            ...
            <span>/ {id ? "Edit User" : "New User"}</span>
          </div>
        </div>
              ...
              <div className="row">
                <div className="col-md-6">
                  <div className="form-group">
                    ...
                    <Select
                      options={Array.isArray(religion) ? religion.map(rel => ({
                        value: rel._id,
                        label: rel.name
                      })) : []}
                      onChange={(option) => {
                        formik.setFieldValue("religion_id", option?.value || "");
                      }}
                      value={Array.isArray(religion) ? religion.find(
                        (option) => option?._id === formik?.values?.religion_id
                      ) : null}
                    ...
                    {formik.touched.religion_id && formik.errors.religion_id ? (
                      <div style={{ color: "red" }}>{formik.errors.religion_id}</div>
                    ) : null}
                  </div>
                </div>
                ...
                <div className="col-md-6">
                  <div className="form-group">
                    ...
                    <Select
                      isMulti
                      options={Array.isArray(languages) ? languages.map(lang => ({
                        value: lang._id,
                        label: lang.name
                      })) : []}
                      onChange={(option) => {
                    ...
                    />
                    {formik.touched.languages_id && formik.errors.languages_id ? (
                      <div style={{ color: "red" }}>{formik.errors.languages_id}</div>
                    ) : null}
                  </div>
                </div>
                ...
                <div className="col-md-6">
                  <div className="form-group">
                    ...
                    <Select
                      isMulti
                      options={Array.isArray(nativeplace.data) ? nativeplace.data.map(place => ({
                        value: place._id,
                        label: place.name
                      })) : []}
                      onChange={(option) => {
                    ...
                    />
                    {formik.touched.nativeplace_id && formik.errors.nativeplace_id ? (
                      <div style={{ color: "red" }}>{formik.errors.nativeplace_id}</div>
                    ) : null}
                  </div>
                </div>
                ...
                <div className="col-md-12">
                  <div className="form-group">
                    ...
                    {formik.touched.significance && formik.errors.significance ? (
                      <div style={{ color: "red" }}>{formik.errors.significance}</div>
                    ) : null}
                  </div>
                </div>
              </div>
