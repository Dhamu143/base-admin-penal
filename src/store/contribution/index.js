...
export const appAllGetContribution = createAsyncThunk(
  "appContribution/appAllGetContribution",
  async (params) => {
    console.log('params', params)
    try {
      let url = `/contribution?page=${params.page}&limit=${params.limit}`;
      if (params.hub) {
        url += `&hub=${params.hub}`;
      }
      if (params.sanstha) {
...
export const appUpdateContribution = createAsyncThunk(
  "appContribution/appUpdateContribution",
  async (params) => {
    // console.log(params, "params")
    try {
      const response = await httpService.put(
        `/contribution/${params.id}`,
        {},
        {
...
export const appDeleteContribution = createAsyncThunk(
  "appContribution/appDeleteContribution",
  async (id) => {
    try {
        const response = await httpService.delete(`/contribution/${id}`);
      if (response?.data) {
        response.data.id = id;
...
export const appContributionSlice = createSlice({
  name: "Contribution",
  initialState: {
    contribution: [],
    isloder: false,
  ...
  extraReducers: (builder) => {
    ...
    builder.addMatcher(isAllOf(appAllGetContribution.pending), (state, action) => {
      state.isloder = true;
    });
    builder.addMatcher(
      isAllOf(appAllGetContribution.fulfilled),
      (state, action) => {
    ...
    builder.addMatcher(
      isAllOf(appAllGetContribution.rejected),
      (state, action) => {
        state.isloder = false;
      }
    );
...
export default appContributionSlice.reducer;